...
\begin{figure}
	\begin{center}
		\begin{tikzpicture}[node distance=1.5cm]
			...
			\node[entity] (cond) [below of = diff] {Rençoit ordre chargement} edge (diff);
			\node[entity] (end) [below of=cond] {Envoit fin chargement} edge (cond);
			\node[entity] (tir) [below of=end] {Reçoit ordre tir} edge (end);
			\node[entity] (score) [below of=tir] {Envoit score} edge (tir);
			\node[relationship] (term) [below of = score,node distance=2cm] {Reçoit fin ?} edge (score);

			% Flèche 1
			\draw (0,-9.2) -- (0,-10);
			\draw (0.5,-9.6) node {Non};
			\draw (0,-10) -- (-3,-10);
			\draw (-3,-10) -- (-3,-1.5);
			\draw[->] (-3,-1.5) -- (-2.3,-1.5);
